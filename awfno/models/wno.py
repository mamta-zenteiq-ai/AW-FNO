...
class WNO1d(nn.Module):
    def __init__(self, in_channels, out_channels, width, size, level=2, n_layers=4, padding=0, wavelet='db4'):
        ...
        self.conv_layers = nn.ModuleList([
            WaveConv1d(width, width, level, padded_size, wavelet=wavelet) 
...
class WNO2d(nn.Module):
    def __init__(self, in_channels, out_channels, width, size, level=2, n_layers=4, padding=2, wavelet='db4'):
...
class WNO3d(nn.Module):
    def __init__(self, in_channels, out_channels, width, size, level=2, n_layers=4, padding=0, wavelet='db4'):
        ...
        lifting_in = in_channels + 3 # +3 for x,y,z grid
        self.fc0 = nn.Linear(lifting_in, self.width)

        padded_size = [s + padding for s in size]
        self.conv_layers = nn.ModuleList([
            WaveConv3d(width, width, level, padded_size, wavelet=wavelet) 
            for _ in range(n_layers)
        ])
        self.w_layers = nn.ModuleList([
    ...
    def get_grid(self, shape, device):
        batchsize, nx, ny, nz = shape[0], shape[2], shape[3], shape[4]
        gridx = torch.linspace(0, 1, nx, device=device).reshape(1, 1, nx, 1, 1).repeat([batchsize, 1, 1, ny, nz])
        gridy = torch.linspace(0, 1, ny, device=device).reshape(1, 1, 1, ny, 1).repeat([batchsize, 1, nx, 1, nz])
        gridz = torch.linspace(0, 1, nz, device=device).reshape(1, 1, 1, 1, nz).repeat([batchsize, 1, nx, ny, 1])
        return torch.cat((gridx, gridy, gridz), dim=1)

    def forward(self, x):
        grid = self.get_grid(x.shape, x.device)
        x = torch.cat((x, grid), dim=1)
        x = F.gelu(self.fc0(x.permute(0, 2, 3, 4, 1))).permute(0, 4, 1, 2, 3)
        if self.padding > 0:
