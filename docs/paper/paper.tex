...
\textbf{The AW-FNO introduces a hybrid neural operator framework that synergistically integrates the global spectral efficiency of FNOs with the multi-resolution localisation of WNOs for super-resolving turbulent flows. The fusion of a parallel kernel architecture and an adaptive gated fusion mechanism lets the model select between the two bases in a data-dependent way, and this selection is itself regime-dependent: on shock-dominated flows the gate routes \emph{spatially}, assigning the wavelet branch to sharp fronts and the Fourier branch to smooth regions, whereas on statistically homogeneous turbulence it instead commits to a near-binary \emph{per-channel} allocation of the two operator sub-spaces applied across the domain. Both modes require a gate with sufficient representational capacity --- a single $1\times1$ convolution collapses to an indecisive near-uniform mix --- and both reduce FNO's Gibbs oscillations and spectral bias near local structure, yielding high-fidelity reconstruction (a $\sim$50\% relative-$L_2$ improvement over the best single operator on the super-resolution benchmark) with reduced spurious high-wavenumber energy compared with standalone neural operators.}
