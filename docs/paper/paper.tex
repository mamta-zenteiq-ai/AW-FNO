...
\]
The kernel size is chosen so that the gate's effective receptive field (a $9$-pixel window for $k=5$ stacked twice) exceeds the characteristic width of the sharp features in the data --- approximately $5$--$10$ grid points for Burgers shocks at $\nu = 10^{-3}$. A single $1\times1$ convolution, having no spatial context, cannot express such a routing pattern and collapses to a near-uniform, indecisive gate; the added depth and spatial receptive field instead let the gate commit decisively to one basis or the other. Whether that commitment is realised as spatial localisation of sharp features or as a per-channel allocation of the two operator sub-spaces is regime-dependent, as analysed in Section~\ref{sec:results-gate}.
...
To probe a second, qualitatively different turbulence regime, we also
evaluate on the Johns Hopkins Turbulence Database isotropic-turbulence
flow (\texttt{isotropic4096}), processed through the identical
$128^2$/$4\times$ SR pipeline (Section~\ref{sec:results-jhtdb}); unlike
the modest-Reynolds forced-NS field, it is strongly intermittent. We
additionally use the PDEBench Burgers ($\nu=10^{-3}$) and 1D shock-tube
(Riemann) flows to study the gate on data with genuine
discontinuities.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    \textbf{AW-FNO (rich gate)}   & 89.3M  & \textbf{0.0090} & \textbf{0.081} & \textbf{111.0} & 2.1e-3  & \textbf{0.017} \\
    \hline
  \end{tabular}
\end{table}
...
receptive field (the rich gate) gives the best pointwise and spectral accuracy in
the table --- relative $L_2 \approx 0.0090$, a $\sim$50\% reduction over
the best single operator (WNO) and $\sim$33\% over the $1\times1$ gate,
together with the lowest relative $H_1$ ($0.081$) and high-frequency
spectral error ($111.0$) --- driven by a sharply decisive gate
($H\approx0.017$). The one exception is the enstrophy error, on which
the rich gate trades a small amount of global-quantity fidelity for its
large pointwise gain. Crucially, as Section~\ref{sec:results-gate}
shows, this decisiveness is realised \emph{per channel} rather than per
location, which reshapes how the gate should be interpreted on
homogeneous turbulence.

% Rich-gate run complete (epoch 500/500, best test rel L2 = 0.008988).
% Table 1 + this section reflect the final aggregate_sr_results.py output.

\subsection{Energy Spectrum}
\label{sec:results-spectrum}

To assess physical fidelity beyond point-wise error, we compute the
radially averaged energy spectrum $E(|k|)$ for each model and compare it
with the ground-truth HR field (Figure~\ref{fig:spectrum}). All methods
reproduce the large-scale (low-$|k|$) energy accurately; the models
separate in the high-$|k|$ band above the LR Nyquist cutoff, the content
that $4\times$ downsampling removes from the input. There, bicubic
interpolation injects substantial spurious energy: the fraction of total
energy lying above the cutoff is $7.3\times10^{-6}$, against
$1.2\times10^{-8}$ for the ground truth. Every trained operator
suppresses this ringing artefact, and the ordering mirrors the
high-frequency $L_2$ column of Table~\ref{tab:sr-main} --- the spurious
high-$k$ fraction falls from $1.8\times10^{-6}$ (FNO) and
$1.1\times10^{-6}$ (WNO) to $1.8\times10^{-7}$ for AW-FNO with the rich
gate, an order of magnitude closer to the ground truth than either
single operator. This is direct evidence for the Gibbs-suppression
mechanism that motivates the wavelet branch (Section~\ref{sec:gate}).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\linewidth]{figures/energy_spectrum.png}
  \caption{Radially averaged energy spectrum $E(|k|)$ on the NS-forcing
  $128^2$ SR test set. All operators match the low-$|k|$ energy; above
  the LR Nyquist cutoff the rich-gate AW-FNO tracks the ground-truth
  tail most closely and injects the least spurious high-wavenumber
  energy, whereas bicubic upsampling overshoots it the most.}
  \label{fig:spectrum}
\end{figure}
...
A natural hypothesis, borne out on shock-dominated data, is that the
spatial gate routes the wavelet branch to sharp features and the Fourier
branch to smooth regions. On a viscous Burgers benchmark
($\nu=10^{-3}$) the same rich gate indeed develops this behaviour, with
$\rho\big((1-\alpha),|\nabla u|\big)$ reaching $+0.32$ along shock
fronts. On the forced-turbulence SR task, however, the picture is
different and instructive. Extracting the per-block gate maps of the
trained rich-gate model and correlating the WNO weight $(1-\alpha)$ with
the vorticity-gradient magnitude $|\nabla\omega|$ of the target yields a
correlation indistinguishable from zero ($\rho\approx-0.004$ on the
deepest block; individual blocks and samples span $[-0.06,+0.10]$). The
channel-averaged gate is spatially almost flat (standard deviation
$\approx2\times10^{-3}$), and overlaying its map on the strongest
vorticity gradients (Figure~\ref{fig:gatemap}b) reveals no
correspondence: on this data the gate does \emph{not} route spatially.

The decisiveness reported in Table~\ref{tab:sr-main} ($H\approx0.017$) is
instead realised along the \emph{channel} axis. Within every fusion
block the $32$ gate channels split into two saturated groups --- about
$12$ channels with $\alpha\approx0$ (assigned to WNO) and $20$ with
$\alpha\approx1$ (assigned to FNO), with essentially none in between
(Figure~\ref{fig:gatemap}a) --- and each channel's gate is near-constant
in space (per-channel spatial standard deviation $\approx6\times10^{-3}$).
The rich gate has therefore learned a hard, near-static partition of the
feature channels between the two operator sub-spaces, applied uniformly
across the domain. The $1\times1$ gate is spatially flat for the same
reason, but, lacking the capacity to commit, leaves all channels near
the indecisive value $\alpha\approx0.5$ ($H\approx0.67$); the rich gate's
advantage comes precisely from converting this indecision into a sharp
channel allocation.

These observations are best read as a \emph{regime characterization}.
When the data contains localised, isolated discontinuities (Burgers and
shock-tube flows), the optimal use of the two bases is spatial, and the
gate routes accordingly. When the data is statistically homogeneous ---
forced two-dimensional turbulence, in which sharp gradients are vortex
filaments distributed throughout the domain rather than confined to a
few fronts --- no localised sub-region uniformly prefers one basis, and
the gate instead exploits the complementary spectral content of the FNO
and WNO sub-spaces through a channel-wise allocation applied everywhere.
Both are adaptive expert-selection strategies, but along different axes
(space versus channel); the accuracy improvement in
Table~\ref{tab:sr-main} confirms that, in this regime, channel
allocation is the effective one.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/gate_mechanism.pdf}
  \caption{Gate behaviour of the rich-gate AW-FNO on the NS-forcing SR
  task. (a) Histogram of per-channel mean gate values in the deepest
  fusion block: the $32$ channels split into a near-binary allocation
  ($\sim$12 to WNO at $\alpha\approx0$, $\sim$20 to FNO at
  $\alpha\approx1$), which accounts for the low gate entropy. (b) The
  channel-averaged spatial gate map is essentially uniform and shows no
  alignment with the strongest vorticity gradients (white contours,
  top-10\% $|\nabla\omega|$): on homogeneous turbulence the gate
  allocates by channel, not by location.}
  \label{fig:gatemap}
\end{figure}


\subsection{Generalisation to a Second Turbulence Regime: JHTDB}
\label{sec:results-jhtdb}

To test whether these findings are specific to the forced-NS data, we repeat
the SR experiment on the Johns Hopkins Turbulence Database isotropic-turbulence
dataset (\texttt{isotropic4096}), a high-Reynolds, strongly \emph{intermittent}
flow in which energy concentrates on thin vortex filaments embedded in a smooth
background. We use the identical $128^2$/$4\times$ pipeline and the focused
four-model set (Table~\ref{tab:jhtdb}).

\begin{table}[t]
  \centering
  \caption{SR on JHTDB isotropic turbulence ($128^2$, $4\times$). The rich-gate
  AW-FNO again attains the lowest error and---unlike on forced NS---its gate
  routes strongly in space.}
  \label{tab:jhtdb}
  \begin{tabular}{lccc}
    \hline
    Model & Rel $L_2$ & Rel $H_1$ & $\rho\big((1-\alpha),|\nabla u|\big)$ \\
    \hline
    FNO                         & 0.0250 & ---    & ---    \\
    WNO                         & 0.0247 & ---    & ---    \\
    AW-FNO (fixed $\alpha{=}0.5$) & 0.0251 & ---  & ---    \\
    \textbf{AW-FNO (rich gate)} & \textbf{0.0232} & 0.325 & \textbf{+0.48} \\
    \hline
  \end{tabular}
\end{table}

The rich gate is again the most accurate model. The decisive difference from
the forced-NS task is the gate's spatial behaviour: the routing correlation
rises to $\rho\approx+0.48$ (Figure~\ref{fig:jhtdb}), the strongest of any
dataset we study and above the Burgers shock value. High-Reynolds isotropic
turbulence is genuinely intermittent---its sharp velocity gradients are
localised on vortex filaments---so here, unlike the homogeneous forced-NS
field, there \emph{are} localised structures for the gate to route to, and it
does.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\linewidth]{figures/jhtdb_gate.png}
  \caption{Rich-gate AW-FNO on JHTDB. The WNO weight $(1-\alpha)$ correlates
  with the velocity-gradient magnitude $|\nabla u|$ of the target ($\rho=+0.58$
  for this sample), raising the wavelet contribution along high-strain vortex
  filaments (white contours: top-10\% $|\nabla u|$).}
  \label{fig:jhtdb}
\end{figure}
...
Table~\ref{tab:regime} collects the rich-gate AW-FNO across the flow regimes we
study. A single pattern organises the results: \emph{adaptive spatial routing
emerges precisely when the flow contains localised sharp structures, and is
absent when it does not}. The gate routes in space on viscous Burgers shocks
($\rho=+0.17$) and most strongly on intermittent high-Re isotropic turbulence
($\rho=+0.48$); it does not route in space on the statistically homogeneous
forced-NS field ($\rho\approx0$), where it instead realises the FNO/WNO
trade-off through a per-channel allocation (Section~\ref{sec:results-gate}). In
every regime the rich-gate AW-FNO is the most (or equal-most) accurate operator,
but the \emph{mechanism} by which it uses the two bases is regime-dependent.
This reframes ``adaptive routing'' from a fixed behaviour into a data-driven
choice between spatial and channel-wise expert selection.

\begin{table}[t]
  \centering
  \caption{Rich-gate AW-FNO across regimes. The spatial-routing strength
  $\rho((1-\alpha),|\nabla\cdot|)$ tracks the spatial intermittency of the
  \emph{flow}, not the model.}
  \label{tab:regime}
  \begin{tabular}{llccc}
    \hline
    Dataset & Regime & Rel $L_2$ & $\rho$ & Gate $H$ \\
    \hline
    Forced NS $128^2$ SR      & homogeneous turbulence & 0.0090 & $\approx 0$ & 0.017 \\
    JHTDB iso.\ SR            & high-Re intermittent   & 0.0232 & $+0.48$     & 0.011 \\
    Burgers $\nu{=}10^{-3}$   & viscous shocks         & 0.033  & $+0.17$     & 0.03  \\
    \hline
  \end{tabular}
\end{table}
...
\textbf{Single-channel (purely spatial) gate.} Can the gate be \emph{forced} to
route spatially by removing its channel axis? We trained a variant whose gate
emits a single map $\alpha(x,y)$ shared across all feature channels, so its only
route to decisiveness is spatial. It is uniformly worse: on forced-NS SR it
reaches Rel $L_2=0.0099$ (vs.\ $0.0090$) and still does not route
($\rho\approx0$), and on Burgers it degrades to $0.053$ (vs.\ $0.033$) while
collapsing to a constant per-block basis choice ($\rho\approx0$). Spatial
routing therefore \emph{emerges from} the per-channel structure rather than
being suppressed by it; forcing a single spatial mask removes the flexibility
that makes the gate effective.

\textbf{Gate kernel size.} Sweeping the gate kernel on Burgers gives Rel $L_2$
of $0.0364, 0.0333, 0.0341, 0.0345$ for $k=3,5,7,9$, with routing
$\rho=+0.14,+0.17,+0.05,+0.10$. $k=5$ is the sweet spot for both accuracy and
routing; enlarging the receptive field past the characteristic feature width
brings no further gain.

\textbf{Inference cost.} Forward latency at $128^2$ is $0.60$\,ms/sample for
FNO, $1.83$ for WNO, and $5.08$ for the rich-gate AW-FNO---an $\sim$$8\times$
overhead over a plain FNO but still well within real-time budgets, for a
$\sim$50\% error reduction.

\textbf{Limitation.} On the PDEBench 1D shock-tube (Riemann) benchmark---predict
the developed state of a random two-state initial condition---all operators
plateau near Rel $L_2\approx0.30$ and the gate does not route ($\rho\approx0$);
this long-horizon map is too hard for the single-step models used here, and we
...
\textbf{The AW-FNO introduces a hybrid neural operator framework that synergistically integrates the global spectral efficiency of FNOs with the multi-resolution localisation of WNOs for super-resolving turbulent flows. The fusion of a parallel kernel architecture and an adaptive gated fusion mechanism lets the model select between the two bases in a data-dependent way, and this selection is itself regime-dependent: where the flow has localised sharp structures --- shock fronts in Burgers and shock-tube flows, or vortex filaments in high-Reynolds intermittent turbulence --- the gate routes \emph{spatially}, assigning the wavelet branch to those structures (the routing correlation reaching $\rho\approx0.48$ on isotropic turbulence); on statistically homogeneous turbulence, by contrast, it commits to a near-binary \emph{per-channel} allocation of the two operator sub-spaces applied across the domain. Both modes require a gate with sufficient representational capacity --- a single $1\times1$ convolution collapses to an indecisive near-uniform mix --- and both reduce FNO's Gibbs oscillations and spectral bias near local structure, yielding high-fidelity reconstruction (a $\sim$50\% relative-$L_2$ improvement over the best single operator on the super-resolution benchmark) with reduced spurious high-wavenumber energy compared with standalone neural operators.}
