"""
2D Darcy flow in triangular domain with notch — AW-FNO version with H1 Sobolev loss.
...
Model: AWFNO2dFinalAGFM
  wavelet=db6, level=3, hidden_channels=64, n_layers=4, epochs=500.
...
Results saved to: results/awfno_darcy_notch/
"""
...
def load_darcy_triangular(mat_path: str, subsample: int = 2):
    ...
    bc  = bc[:,  ::subsample, ::subsample]    # (N, 51, 51)
    sol = sol[:, ::subsample, ::subsample]    # (N, 51, 51)
...
def train_awfno_darcy_notch():
    set_seed(42)
    device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")

    # -----------------------------------------------------------------------
    # Hyperparameters  (matching WNO paper Table 1 / Table 2 where applicable)
    # -----------------------------------------------------------------------
    epochs      = 500
    batch_size  = 20
    lr          = 1e-3
    ...
    results_dir = os.path.join(PROJECT_ROOT, 'results', 'awfno_darcy_notch')
    ...
    # y_test is kept in physical units for evaluation
    ...
    # -----------------------------------------------------------------------
    # Model — AWFNO2dFinalAGFM
    # Paper Table 1: wavelet=db6, level m=3, 4 WNO layers
    # n_modes=(12,12): keeps up to 12 Fourier modes per spatial dim (≤ 51//2=25)
    # -----------------------------------------------------------------------
    # padding=1: pads 51×51 → 52×52 internally so the DWT filterbank
    # sees an even-sized grid (avoids the off-by-one size mismatch that
    # arises when pytorch_wavelets zero-pads odd inputs before IDWT).
    model = AWFNO2dFinalAGFM(
        in_channels=1,
        out_channels=1,
        n_modes=(12, 12),
    ...
    print(f"\nStarting AW-FNO Darcy-notch training for {epochs} epochs...")
    print(header)
    print("-" * len(header))
    ...
    # -----------------------------------------------------------------------
    ckpt_path = os.path.join(results_dir, 'awfno_darcy_notch_model.pt')
    ...
    axes[0].set_title('AW-FNO Darcy Notch — Sobolev H1 Loss')
    ...
    axes[1].set_title('AW-FNO Darcy Notch — MSE')
    ...
    axes[2].set_title('AW-FNO Darcy Notch — Relative L2')
    axes[2].legend(); axes[2].grid(True, alpha=0.3)

    plt.suptitle(
        f'AW-FNO Darcy Triangular (Notch)  |  '
        f'Final Test Rel-L2: {test_rel_hist[-1]:.4f}  |  '
        f'Final Test H1: {test_h1_hist[-1]:.4e}',
        fontsize=11
    )
    plt.tight_layout()
    loss_plot = os.path.join(results_dir, 'awfno_darcy_notch_loss.png')
    ...
    col_titles = ['Ground Truth', 'AW-FNO Prediction', 'Absolute Error']
    ...
    fig.suptitle(
        f'AW-FNO — Darcy Triangular Notch: GT | Prediction | Error\n'
        f'Mean Test Rel-L2: {test_rel_hist[-1]:.4f}',
        fontsize=14
    )
    plt.tight_layout()
    field_plot = os.path.join(results_dir, 'awfno_darcy_notch_fields.png')
    ...
    # -----------------------------------------------------------------------
    print(f"\n{'='*70}")
    print(f"  AW-FNO Darcy Notch — Training Summary")
    print(f"{'='*70}")
