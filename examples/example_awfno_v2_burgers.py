"""
Training script for AW-FNO v2 on the 1-D Burgers equation.

Uses the same data pipeline, optimizer, scheduler, loss and evaluation
as the FNO / WNO / AW-FNO v1 Burgers scripts for a fair comparison.
"""
...
def train_burgers():
    ...
    results_dir = os.path.join(PROJECT_ROOT, 'results', 'awfno_v2_burgers')
    ...
    # Ensure channel dimension: (N, 128) → (N, 1, 128)
    if x_train.ndim == 2:
    ...
    print(f"AWFNOv2_1d  —  trainable parameters: {n_params:,}")
    ...
    print(f"Starting AW-FNO v2 training on Burgers for {epochs} epochs...")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MSE')
    plt.title('AW-FNO v2 Burgers MSE Loss')
    plt.legend()
    ...
    plt.title('AW-FNO v2 Burgers Relative L2 Loss')
    plt.legend()

    plt.tight_layout()
    plot_path = os.path.join(results_dir, 'awfno_v2_burgers_loss_plot.png')
