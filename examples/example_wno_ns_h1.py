...
def train_wno_ns_h1():
    ...
    results_dir = os.path.join(PROJECT_ROOT, 'results', 'wno_ns_h1')
    ...
    # 2. Load Data
    print("Loading Navier-Stokes (128x128) data for WNO-H1...")
    ...
    print(f"Starting WNO 2D training (Sobolev H1 Loss) on Navier-Stokes for {epochs} epochs...")
    ...
    plt.title('WNO NS Test Performance')
    plt.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'wno_ns_h1_loss_plot.png'))
    ...
    with torch.no_grad():
        ...
        plt.tight_layout()
        field_plot_path = os.path.join(results_dir, 'wno_ns_h1_field_comparison.png')
