"""
Burgers equation with discontinuity in the velocity field — WNO version.
...
WNO-specific note on wavelet level:
    WaveConv2d performs a 2D DWT over the (N_x, T_in) = (512, 10) grid.
    With the Daubechies-6 wavelet (filter length 12), one level of DWT halves
    both dimensions: 512 → ≈256, 10 → ≈5.
    Level 2 would further halve 5 → ≈2.5 (non-integer, unsafe), so level=1 is
    the maximum reliable decomposition depth for T_in=10.
    To use level=2 or higher, increase T_in to at least 16.
"""
...
# ---------------------------------------------------------------------------
# WNO2d treats the input (B, C, N_x, T_in) as a 2D spatial-temporal field.
# Internally it appends 2D grid coordinates (x, t normalised to [0,1]) and
# performs multi-level 2D DWT on the (N_x, T_in) grid before kernel learning.
# ---------------------------------------------------------------------------
...
def train_burgers_discontinuity():
    ...
    # WNO paper hyperparameters for the Burgers discontinuity case
    epochs       = 500
    batch_size   = 20
    lr           = 1e-3
    print_every  = 50
    ...
    results_dir = os.path.join(PROJECT_ROOT, 'results', 'wno_burgers_discontinuity')
    ...
    # The 1D Burgers problem is broadcast to 2D by treating the time-window
    # axis as the second spatial axis. WNO2d then applies 2D DWT over the
    # (N_x=512, T_IN=10) grid and learns wavelet-space kernels.
    # -----------------------------------------------------------------------
    ...
    y_test  = test_data[:, :, T_IN:T_IN + T_OUT]      # (100, 512, 40)
    ...
    # -----------------------------------------------------------------------
    # Model — WNO2d with 2D wavelet convolution over the (x, t) grid.
    # size  : (N_x, T_IN) = (512, 10) — 2D grid dimensions passed to WaveConv2d.
    #         WaveConv2d uses this to initialise weight tensors via a dummy DWT.
    # level : 1  — maximum safe level for T_IN=10 with db6 wavelet
    #         (level 2 would attempt to halve 5 → 2.5, which is non-integer).
    # width : latent channel width (= hidden_channels in other models).
    # padding: WNO2d pads (N_x, T_IN) → (N_x+p, T_IN+p) before WaveConv2d.
    #          padding=2 would enlarge the DWT input to (514,12), making each
    #          weight_approx tensor (32,32,257,6) instead of (32,32,256,5) — only
    #          a minor size difference with width=32, but kept at 0 for simplicity
    #          and to match the original grid dimensions exactly.
    # -----------------------------------------------------------------------
    model = WNO2d(
        in_channels=1,
        out_channels=1,
        width=32,             # width=64 made weight tensors ~100M params total → slow backward
        size=(N_X, T_IN),     # (512, 10) — 2D grid: (spatial, temporal window)
        level=1,              # level=1 required; see module docstring for constraint
        n_layers=4,
        padding=0,            # 0 keeps DWT input at (512,10); avoids extra weight params
    ).to(device)

    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")

    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-4)
    # WNO paper scheduler: decay at rate 0.75 every 50 epochs
    ...
    print(f"Starting WNO training for {epochs} epochs...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for bx, by in train_loader:
            ...
            # MSE drives training for WNO (matching WNO paper baseline).
    ...
    torch.save(model.state_dict(), os.path.join(results_dir, 'wno_burgers_disc_model.pt'))
    ...
    axes[0].set_title('WNO Burgers Discontinuity — MSE')
    ...
    axes[1].set_title('WNO Burgers Discontinuity — Relative L2')
    axes[1].legend(); axes[1].grid(True, alpha=0.3)

    plt.tight_layout()
    loss_plot = os.path.join(results_dir, 'wno_burgers_disc_loss.png')
    ...
    im1 = axes[1].imshow(sample_pred, origin='lower', aspect='auto',
                          extent=[t_coords[0], t_coords[-1], x_coords[0], x_coords[-1]],
                          vmin=vmin, vmax=vmax, cmap='RdBu_r')
    axes[1].set_title('WNO Prediction'); axes[1].set_xlabel('t'); axes[1].set_ylabel('x')
    ...
    fig.suptitle(f'WNO Burgers Discontinuity  |  Test Rel L2: {test_rel_hist[-1]:.4f}')
    plt.tight_layout()
    heatmap_plot = os.path.join(results_dir, 'wno_burgers_disc_heatmap.png')
    plt.savefig(heatmap_plot, dpi=150)
    plt.close()
    print(f"Spatiotemporal heatmap saved: {heatmap_plot}")

    # Temporal snapshot comparisons (4 time instants, like Figure 5 in the WNO paper)
    ...
    for ax, idx, t_snap in zip(axes, snap_indices, snap_times):
        ...
        ax.plot(x_coords, sample_pred[:, idx], '--', color='crimson', lw=2, label='WNO')
        ax.set_title(f't = {t_snap:.2f} s')
        ax.set_xlabel('x')
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.3)
    axes[0].set_ylabel('u(x, t)')
    fig.suptitle('WNO Burgers Discontinuity — Temporal Snapshots')
    plt.tight_layout()
    snap_plot = os.path.join(results_dir, 'wno_burgers_disc_snapshots.png')
