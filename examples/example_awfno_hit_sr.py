...
def train_awfno_hit_sr():
    ...
    # 2) 5-panel: GT | Raw LR (32×32) | Bicubic-upsampled LR | AW-FNO Prediction | |Error|
    ...
    # - x_test_in_phys is the bicubic-upsampled LR (physical scale).
    ...
    fig.suptitle(f'AW-FNO HIT SR ({lr_train}→{hr_size}) — GT | LR | Bicubic LR | Prediction | |Error|',
    ...
    for r, i in enumerate(idx):
        ...
        err   = np.abs(gt - pr)
        # Raw LR (32x32) in physical scale: avg-pool the HR GT with the same kernel
        # the training pipeline uses (hr_size // lr_train = 4 for 32→128).
