...
def train():
    ...
    results_dir = os.path.join(PROJECT_ROOT, 'results', 'fno_only')
    os.makedirs(results_dir, exist_ok=True)
    
    # 2. Load Data
    print("Loading data for FNO training...")
    ...
    print(f"Starting FNO-only training for {epochs} epochs...")
    ...
    print(f"FNO Training completed in {total_time:.2f}s")
    ...
    plt.title('FNO Relative L2 Loss')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'fno_loss_plot.png'))
    print(f"Loss plot saved to {os.path.join(results_dir, 'fno_loss_plot.png')}")
