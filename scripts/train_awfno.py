...
def train():
    ...
    # We will normalize y during training loop or before
    # For now, let's normalize everything
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_x, batch_y in train_loader:
            ...
            # For relative L2, we decode to compare in original space
    ...
    plt.title('Relative L2 Loss')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'loss_plot.png'))
    print(f"Loss plot saved to {os.path.join(results_dir, 'loss_plot.png')}")
